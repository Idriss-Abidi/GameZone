...
const CardGrid = () => {
  ...
  return (
    <div id="EVENTS" className="p-4 relative">
      {/* Timer */}
      ...
      {/* Title and Paragraph */}
      <motion.h2 whileInView={{ opacity: 1, y: 0 }} initial={{ y: -100, opacity: 0 }} transition={{ duration: 1.5 }} className="my-20 text-center text-4xl">
        CODE NAMES
      </motion.h2>
      <motion.p
      ...
      >
        <p className="text-lg mb-4">
          You need to find the correct 7 words in 60 seconds based on the hints that we will give you once you click the START button! <br/>
          So take your time to read the words before starting the game.
        </p>
        <button
          className={`bg-blue-500 text-white py-2 px-4 rounded-lg shadow-lg hover:bg-blue-600 transition duration-300 ${gameStarted || tries <= 0 ? 'opacity-50 cursor-not-allowed' : ''}`}
      ...
      {gameOver && (
        ...
        >
          <div className="text-center text-white">
            <h2 className="text-2xl font-bold mb-4">{success ? `Congrats, You solved it with time rest = ${time}s` : 'Game Over!'}</h2>
            <p className="text-lg mb-4">{success ? 'The hint is "Address"' : 'Better luck next time!'}</p>
            <button
